  ...
  const handleGeneratePayroll = async () => {
    try {
      ...
      if (!generateAllEmployees && !employeeID) {
        console.error(
          'Please choose either "Generate All Employees" or specify an Employee ID.'
        );
        return;
      }
      ...
      const processedData = Object.values(payrollData).map((employee) => {
        const salaryPerMonth =
          ...
          )?.salaryPerMonth || 0; // Retrieving salaryPerMonth
        ...
        const pagibigDeductionPercentage =
          activeEmployeesData.find(
            (item) => item.employeeID === employee.employeeID
          )?.pagibigDeduction || 0; // Retrieving pagibig deduction percentage
        ...
        const philhealthDeductionPercentage =
          activeEmployeesData.find(
            (item) => item.employeeID === employee.employeeID
          )?.philhealthDeduction || 0; // Retrieving philhealth deduction percentage
        const philhealthDeduction =
          grossPay * (philhealthDeductionPercentage / 100); // Calculating philhealth deduction amount

        const sssDeductionPercentage =
          activeEmployeesData.find(
            (item) => item.employeeID === employee.employeeID
          )?.sssDeduction || 0; // Retrieving SSS deduction percentage
  ...
  return (
    <>
      ...
      <div className="payroll-container my-4 font-inter text-sm mx-4">
        ...
        <div className="employee-id-input flex flex-row justify-center w-full">
          <label
            htmlFor="employee-id"
